...
const imagesListNew = [...imagesList]
...
class App extends Component {
  ...
  onShuffledImagesList = list => list.sort(() => Math.random() - 0.5)

  onClickThumbnail = id => {
    const {score, list} = this.state
    const shuffledImagesList = this.onShuffledImagesList(list)
    if (imagesList[score].thumbnailUrl === id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        list: shuffledImagesList,
      }))
    } else {
      this.setState(prevState => ({
  ...
  renderOutputFinal = () => {
    ...
    return (
      <div className="main_container_win_image_container main_container_win_image">
        <div className="main_container_win_image_container_sub">
          <img
            alt="trophy"
            className="win_image"
            src="https://assets.ccbp.in/frontend/react-js/match-game-trophy.png"
          />
          <p className="win_para_one">YOUR SCORE</p>
          <p className="win_para_two">{score}</p>
          <button
          ...
          >
            ...
            />
            <p className="reset_para">PLAY AGAIN</p>
          </button>
        </div>
      </div>
    )
  ...
  renderInput = () => {
    const {score, category, list} = this.state
    const a = imagesList[score].imageUrl
    const newList = list.filter(eachItem => eachItem.category === category)
    return (
      <div className="renderInputContainer">
        <ul className="useLessList">
          <li className="liss">
            <img className="renderInputMainImage" alt="match" src={a} />
          </li>
        </ul>
        <ul className="tabsListUnOrder">
          {tabsList.map(eachItem => {
            const addStyle =
              category === eachItem.tabId
                ? 'tabsListUnOrderSingle button_down_border'
                : 'tabsListUnOrderSingle'
            const onClickCategoryChange = () => {
              this.onClickCategory(eachItem.tabId)
            }
            return (
              <li
                key={eachItem.tabId}
                className={addStyle}
                id={eachItem.tabId}
                onClick={onClickCategoryChange}
              >
                <button
                  type="button"
                  className="useless_button tabsListUnOrderSingleTwo"
                >
  ...
  renderCheck = () => {
    ...
    if (score === 30 || time === 0) {
      a = true
    } else if (score === 30 && time === 0) {
      a = true
    }
    const answer = a ? this.renderOutputFinal() : this.renderInput()
    return answer
  }
    ...
    return (
      <div className="background_container">
        <ul className="nav_container">
          ...
          <li className="nav_rightEnd_container">
            <p className="nav_rightEnd_container_score">
              Score:{' '}
              <span className="nav_rightEnd_container_score_span">{score}</span>
            </p>
            <img
              alt="timer"
              className="nav_rightEnd_container_timer_image"
              src="https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png"
            />
            <p className="nav_rightEnd_container_time">{time} sec</p>
          </li>
        </ul>
        <div className="main_container">{this.renderCheck()}</div>
      </div>
    )
  }
}
